...
def moe_forward_tilelang_routed(
    ...
    def kernel(
        ...
        # Fused compute: gate/up and down in one kernel to avoid global up_logits write/read.
        with T.Kernel(M, T.ceildiv(dhidden, block_dhidden), threads=threads) as (bx, by):
            ...
            for e in T.Grid(T.ceildiv(dexpert, block_dexpert)):
                ...
                for k in T.Pipelined(T.ceildiv(dhidden, block_dhidden), num_stages=num_stages):
                    ...
                    T.copy(
                        routed_expert_gate[
                            cur_group_idx,
                            e * block_dexpert : (e + 1) * block_dexpert,
                ...
                T.gemm(up_logits_local, routed_expert_down_shared, output_local, transpose_B=True)
