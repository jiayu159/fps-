...
SENSITIVITY = 0.3       # 灵敏度参数 (0.1-1.0)
CENTER_THRESHOLD = 10      # 中心点阈值(像素)，小于此值认为已瞄准

# 瞄准稳定性参数
AIM_STABILIZATION_TIME = 0.3  # 瞄准稳定时间(秒)，增加这个值可以让瞄准更稳定
STABILIZATION_MOVEMENT_SCALE = 0.3  # 稳定阶段的移动缩放因子(0-1)
MIN_STABILIZATION_FRAMES = 3    # 最小稳定帧数要求
...
CIRCLE_RADIUS = 700          # 圆形区域半径

# 防止连续移动
last_move_time = 0
MOVE_COOLDOWN = 0.5  # 冷却时间

# 性能优化参数
PROCESSING_INTERVAL = 0.03 # 处理间隔增加，提高帧率稳定性
